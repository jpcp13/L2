...
#d 
fi(n)= fi(p)*fi(q)*pgcd(p,q)/fi(pgcd(p,q)) ici pgcd(p,q) = fi(pgcd(p,q)) = 1 car p et q sont premiers donc sont premiers entre eux. 
On en deduit que fi(n) = fi(p)fi(q) = (p-1)(q-1)"
