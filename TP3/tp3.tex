...
\begin{parts}
\part
Calculer, à la main, la solution exacte de l'EDO (\ref{harmo_1}).
\part
Ecrire la fonction $F$ de l'EDO (\ref{harmonic}) dans une fonction python \texttt{F}; on choisira $\omega = 1.0$.
\texttt{F} prendra en arguments un flottant \texttt{t}, un numpy array \texttt{U} et renverra le numpy array \texttt{U'}.
\part
Modifier la fonction \texttt{euler}, pour qu'elle prenne quatre arguments : une fonction python \texttt{F} - elle même prenant deux arguments \texttt{t} flottant et \texttt{U} numpy array -, une valeur initiale \texttt{U0} numpy array, un flottant \texttt{T} et un entier \texttt{n}; \texttt{euler} renverra deux \texttt{numpy arrays} :  \texttt{tt} contenant les valeurs \texttt{tk} et  \texttt{UU} contenant les valeurs  \texttt{Uk} obtenues par la méthode d'Euler.
\part
